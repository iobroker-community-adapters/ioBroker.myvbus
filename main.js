...
class MyVbus extends utils.Adapter {
    ...
    async onReady() {
        try {
            ...
            // test if config is valid. Terminate adapter if not, because it will crash with invalid config
                ...
                ctx.hsc.on('headerSet', () => {
                    ...
                    _.forEach(data, (item) => {
                        ...
                        if (item.name.substring(0, 15) === 'Drehzahl Relais') {
                            this.createOrExtendObject(objectId + '_1', {type:'state',
                                common:{ name: item.name.substring(9, 19) + ' aktiv',
                                    type: 'boolean',
